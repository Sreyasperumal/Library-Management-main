...
function Edit_Book() {
    ...
    initialValues: {
      ...
      validate: (values) => {
        ...
        if (!values.publisher) {
          ...
          values.publisher.length < 3 ||
          values.publisher.length > 15
        ) {
      ...
      onSubmit: async (values) => {
        try {
          let bookEdit = await axios.put(
            `https://6384c57d3fa7acb14f0017a5.mockapi.io/books/${params.id}`,
            values
          );
          if (bookEdit) {
  ...
  return (
      ...
      <div className="container-fluid px-4">
        <h1 className="mt-4">Edit Book Details</h1>
        <form
